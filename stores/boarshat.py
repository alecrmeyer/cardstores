...
class BoarsHat:
    url_base = 'https://www.boarshatgaming.com/products/search?q='

    # Convert card text to the url 
    def query_to_url(self, url, query):
        ret = url + query.replace(" ", "+")
        return ret
    ...
    def get_card(self, card_name):
        ...
        for card in all_items:
            ...
            if name != None and (name.text == card_name or (card_name + " -") in name.text):
                variants = card.find('div', class_='variants')
                quantities = variants.find('span', class_ = 'variant-short-info variant-qty')
